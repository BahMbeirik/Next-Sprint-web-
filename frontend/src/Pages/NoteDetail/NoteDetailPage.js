  ...
  useEffect(() => {
    const fetchUserAndNote = async () => {
      try {
        ...
        const noteResponse = await axios.get(`http://localhost:8000/api/notes/${id}/`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        });
    ...
    <div className="note-container">
     ...
     <span className="button-group">
        {(user && (user.is_staff || userId === noteOwner)) && (
          <>
            <Link to={`/notes/${note.id}/edit`}><button className="btn btn-primary"><FiEdit /><span>Edit</span></button></Link>
