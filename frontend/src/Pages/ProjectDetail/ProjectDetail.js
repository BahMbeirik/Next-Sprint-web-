...
const ProjectDetail = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8000/api/projects/${id}/`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } })
      .then(response => {
    ...
    <div>
      <div className=" m-5 single-note-item all-category">
        <div className="rounded card card-body m-5 p-5">
          ...
          <Link className='d-flex justify-content-center' to={`/projects/${id}/notes`}><button className="btn btn-primary btn-md " type="button">Voir les Sprintes</button></Link>
        </div>
      </div>
    </div>
  );
